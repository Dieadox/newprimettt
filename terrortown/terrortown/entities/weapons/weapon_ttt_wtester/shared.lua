...
if SERVER then
	include( "../../../gamemode/eventlog_settings.lua" )
...
function SWEP:AddPlayerSample(corpse, killer)
   if #self.ItemSamples < self.MaxItemSamples then
      ...
      if not table.HasTable(self.ItemSamples, prnt) then
		...
		if self.Owner == killer then
			str = pref_dna.." "..CreateEvent( str_dna_self, _, _, _, self.Owner, _, _ ).." "..CreateEvent( str_dna_body, _, _, _, _, CORPSE.GetPlayerNick(corpse), _ ) 
		else
			str = pref_dna.." "..CreateEvent( str_dna, _, killer, _, self.Owner, _ ).." "..CreateEvent( str_dna_body, _, _, _, _, CORPSE.GetPlayerNick(corpse), _ ) 
		end
		 LogEvent( str )
