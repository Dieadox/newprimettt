...
function SWEP:PrimaryAttack()
   ...
   owner:ViewPunch( Angle( math.Rand(0.5,1) * self.Primary.Recoil, math.Rand(0.5,1) * self.Primary.Recoil, -0.7 ) )
...
function SWEP:SecondaryAttack()
   ...
   if tr.Hit and IsValid(tr.Entity) and tr.Entity:IsPlayer() and (self.Owner:EyePos() - tr.HitPos):Length() < 100 then
      ...
      if SERVER and (not ply:IsFrozen()) then
		 ...
		 local owner = self.Owner
		 owner:ViewPunch( Angle( math.Rand(1,2) * self.Primary.Recoil, math.Rand(1,2) * self.Primary.Recoil, -1 ) )
