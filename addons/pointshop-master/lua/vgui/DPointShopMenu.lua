...
function PANEL:Init()
	...
	if (PS.Config.AdminCanAccessAdminTab and LocalPlayer():IsAdmin()) or (PS.Config.SuperAdminCanAccessAdminTab and LocalPlayer():IsSuperAdmin()) then
		...
		ClientsList:AddColumn('Coins'):SetFixedWidth(60)
		...
		ClientsList.OnClickLine = function(parent, line, selected)
			...
			menu:AddOption('Set Coins...', function()
				Derma_StringRequest(
					"Set Coins for " .. ply:GetName(),
					"Set coins to...",
					"",
					function(str)
			...
			menu:AddOption('Give Coins...', function()
				Derma_StringRequest(
					"Give Coins to " .. ply:GetName(),
					"Give coins...",
					"",
					function(str)
						if not str or not tonumber(str) then return end
			...
			menu:AddOption('Take Coins...', function()
				Derma_StringRequest(
					"Take Coins from " .. ply:GetName(),
					"Take coins...",
					"",
					function(str)
						if not str or not tonumber(str) then return end
...
function PANEL:Paint()
	...
	surface.DrawTexturedRect( self:GetWide() - 45, 50, 40, 40 )
	
	draw.SimpleText('CoinShop', 'PS_Heading', 20, 10, color_white)
	draw.SimpleText('' .. LocalPlayer():PS_GetPoints() .. '', 'PS_Heading3', self:GetWide() - 50, 62, color_white, TEXT_ALIGN_RIGHT, TEXT_ALIGN_BOTTOM)
end
